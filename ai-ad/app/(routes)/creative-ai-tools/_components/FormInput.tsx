...
import { ImagePlus, Smartphone, Square, Monitor, Loader2Icon } from 'lucide-react';
...
function FormInput({ onHandleInputChange, onGenerate, loading }: Props) {
  ...
  // Convert sample product to File
  const fetchSampleAsFile = async (url: string) => {
    const res = await fetch(url);
    const blob = await res.blob();
    return new File([blob], "sample.png", { type: blob.type });
  };

  // Handle click on sample product
  const handleSampleClick = async (product: string) => {
    setPreview(product);
    setFileReady(false);

    const file = await fetchSampleAsFile(product);
    onHandleInputChange('file', file);
  ...
  return (
    <div>
      ...
      <div className='mt-8'>
        ...
        <Textarea
          placeholder="Tell me more about product and how you want to display it."
      ...
      {/* Generate button */}
      <Button
        disabled={loading || !fileReady} // Disable until file is ready
        className="mt-5 w-full"
        onClick={onGenerate}
      >
        {loading ? <Loader2Icon className="animate-spin" /> : <Sparkle />} Generate
      </Button>
